...
impl Shader {
    ...
    pub fn new(vertex_path: &str, fragment_path: &str) -> Shader {
        ...
        let mut vertex_file =
            File::open(vertex_path).unwrap_or_else(|_| panic!("failed to open file: {}", vertex_path));

        let mut fragment_fiile = 
            File::open(fragment_path).unwrap_or_else(|_| panic!("failed to open file: {}", fragment_path));

        let mut vertex_code = String::new();
        let mut fragment_code = String::new();

        vertex_file.read_to_string(&mut vertex_code).expect("faild to read vertex shader file");
        fragment_fiile.read_to_string(&mut fragment_code).expect("faild to read fragment shader file");
        ...
        if type_ != "PROGRAM" {
            ...
            if success != gl::TRUE as GLint {
                ...
                let info_log_string = match String::from_utf8(info_log) {
                    Ok(log) => log,
                    Err(vec) => panic!(
                        "faildiled to convert to compilation log from buffer: {}",
                        vec
                    ),
                };
                println!(
            ...
            if success != gl::TRUE as GLint {
                ...
                let info_log_string = match String::from_utf8(info_log) {
                    Ok(log) => log,
                    Err(vec) => panic!("faildiled to convert to link log from buffer: {}", vec),
                };
                println!(
    ...
    pub fn with_geomerty_shader(
        ...
        let mut geomerty_file = File::open(geometry_path)
        ...
        let cstr_vertex_code = CString::new(vertex_code.as_bytes()).unwrap();
        let cstr_fragment_code = CString::new(fragment_code.as_bytes()).unwrap();
        let cstr_geometry_code = CString::new(geometry_code.as_bytes()).unwrap();
